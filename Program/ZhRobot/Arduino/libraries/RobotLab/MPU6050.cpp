...
#define WRIE_WRITE(w) Wire.write(w)
#else
#define WIRE_READ() Wire.receive()
#define WRIE_WRITE(w) Wire.send(w)
#endif    
extern uint8_t MPU6050_init_flag;
MPU6050::MPU6050(int sdaport,int sclport) 
{
#ifdef SoftwareI2C_H
  if(sdaport>=0&&sclport>=0)
...
#endif
	{
     ...
     //Serial.print(" [");Serial.print(sda_pin);Serial.print(" ; ");Serial.print(scl_pin);Serial.print("] ");
}
void MPU6050::setpin(int sdaport,int sclport) 
{
#ifdef SoftwareI2C_H
  if(sdaport>=0&&sclport>=0)
	{
...
void MPU6050::init(int sdaport,int sclport) 
{
#ifdef SoftwareI2C_H
  if(sdaport>=0&&sclport>=0)
	{
 ...
 // Initialize MPU6050
    begin(MPU6050_SCALE_2000DPS, MPU6050_RANGE_2G);
  ...
  if(sda_pin>=0&&scl_pin>=0)
	{
		...
		iic.beginTransmission(mpuAddress);
		iic.write(MPU6050_REG_ACCEL_XOUT_H);
		if(iic.endTransmission() != 0) MPU6050_init_flag=0;

        //iic.beginTransmission(mpuAddress);
        iic.requestFrom(mpuAddress);
        xha = iic.receive();
        xla = iic.receive();
        yha = iic.receive();
        yla = iic.receive();
...
#endif
{
    ...
    WRIE_WRITE(MPU6050_REG_ACCEL_XOUT_H);
  ...
  if(sda_pin>=0&&scl_pin>=0)
	{
		SoftwareI2C iic(sda_pin,scl_pin);
		iic.beginTransmission(mpuAddress);
		iic.write(MPU6050_REG_GYRO_XOUT_H);
		if(iic.endTransmission() != 0) MPU6050_init_flag=0;
...
#endif
{
    //Serial.print(" hiic :");
    Wire.beginTransmission(mpuAddress);
    WRIE_WRITE(MPU6050_REG_GYRO_XOUT_H);
    Wire.endTransmission();
  ...
  if(sda_pin>=0&&scl_pin>=0)
	{
		...
		iic.beginTransmission(mpuAddress);
		iic.write(reg);
        iic.write((uint8_t)(value >> 8));
	    iic.write((uint8_t)value);
		if(iic.endTransmission() != 0) MPU6050_init_flag=0;
	}
	else
#endif
{
    Wire.beginTransmission(mpuAddress);
	WRIE_WRITE(reg);
	WRIE_WRITE((uint8_t)(value >> 8));
	WRIE_WRITE((uint8_t)value);
    Wire.endTransmission();
}
}
